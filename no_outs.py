...
model_names = {'LogReg' : lambda df, df_name: tools.LogReg(df, df_name, kind, C),
                'DecTree' : lambda df, df_name : tools.DecTree(df, df_name, kind, crit, depth, max_leaves), 
                'RandForest' : lambda df, df_name : tools.RandForest(df, df_name, kind, crit, n_estimators, max_leaves, depth), 
                'GaussNB' : lambda df, df_name : tools.GaussNB(df, df_name, kind), 
                'NN' : lambda df, df_name : tools.NN(df, df_name, kind, alphas, activation)}

#Load the data sets
df_names = ['GiveMeSomeCredit', 'HomeCredit', 'CreditApproval', 'CreditCardTaiwan', 'SouthGermanCredit']
