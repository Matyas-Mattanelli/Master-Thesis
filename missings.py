...
model_names = {'LogReg' : lambda df, df_name: tools.LogReg(df, df_name, kind, C, missings = True),
                'DecTree' : lambda df, df_name : tools.DecTree(df, df_name, kind, crit, depth, max_leaves, missings = True), 
                'RandForest' : lambda df, df_name : tools.RandForest(df, df_name, kind, crit, n_estimators, max_leaves, depth, missings = True), 
                'GaussNB' : lambda df, df_name : tools.GaussNB(df, df_name, kind, missings = True), 
                'NN' : lambda df, df_name : tools.NN(df, df_name, kind, alphas, activation, missings = True)}

#Load the data sets
df_names = ['GiveMeSomeCredit', 'HomeCredit', 'CreditApproval']
df_paths_train = [f'D:/My stuff/School/Master/Master Thesis/Data/Train data/{i}_{kind}.csv' for i in df_names]
df_paths_test = [f'D:/My stuff/School/Master/Master Thesis/Data/Test data/{i}_{kind}.csv' for i in df_names]
...
for idx, df_path_test in enumerate(df_paths_test): #Loop through the data sets
    ...
    for model_name in model_names: #Loop through the models
        ...
        if evaluation:
            #Get predictions
            preds_binned = tools.predict_binned(model_binned, df_test.loc[:, indep_vars])
            preds_raw = model_raw.predict_proba(df_test.loc[:, indep_vars])[:, 1]

            #Get evaluation metrics
            eval_binned = tools.Evaluate(df_test['Target'], preds_binned, train_means.loc[f'{df_names[idx]}_{kind}'].values[0]) #Binned data
            eval_raw = tools.Evaluate(df_test['Target'], preds_raw, train_means.loc[f'{df_names[idx]}_{kind}'].values[0]) #Raw data
    ...
    if evaluation:
        ...
        with open(f'Evaluation/Interm_{kind}.pkl', 'wb') as out:
            pickle.dump([pd.DataFrame(evals, index = indices), pd.DataFrame(perm_tests, index = indices_perm)], out)
